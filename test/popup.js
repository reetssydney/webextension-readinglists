...
describe('popup.js', () => {
    ...
    it('produces expected POST body', () => {
        assert.deepEqual(getAddToListPostBody(new URL('https://en.wikipedia.org/wiki/Foobar')),
            'project=https://en.wikipedia.org&title=Foobar');
        assert.deepEqual(getAddToListPostBody(new URL('https://de.m.wikipedia.org/wiki/Otto_Wagner')),
            'project=https://de.wikipedia.org&title=Otto_Wagner');
        assert.deepEqual(getAddToListPostBody(new URL('https://fr.wikipedia.org/w/index.php?title=Paso_Fino')),
            'project=https://fr.wikipedia.org&title=Paso_Fino');
    });

    it('parses default list id', () => {
        const response = {
            lists: [
                {
                    id: 4,
                    ...
                    default: true,
                    description: "",
                    created: "2018-04-09T14:23:48Z",
                    updated: "2018-04-09T14:23:48Z"
                },
                {
                    id: 5,
                    name: "Best wiki pages ever",
                    default: false,
                    description: "the cream of the crop",
                    created: "2018-04-10T14:23:48Z",
                    updated: "2018-04-10T14:23:48Z"
                }
            ]
        };
        assert.deepEqual(parseDefaultListId(response), 4);
    });
